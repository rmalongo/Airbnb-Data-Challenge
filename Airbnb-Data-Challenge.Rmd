---
title: "Airbnb Data Challenge: Rio de Janeiro, Brazil"
author: "XXX"
date: "2/1/2020"
output:
...
# Metric 1: Booking conversion rate - contact_me
```{r metric_1}
...
# Metric 2: Booking conversion rate - book_it
```{r metric_2}
...
# Product improvements & business initiatives
## Feature engineering
```{r fet_engin}
  # Feature engineering 
...
a) The features provided may not necessarry be the best learners:
with GBM, these week learners are converted to strong learners
b) GBM offers best prodictive accauracy
c) GBM Handles missing data - imputation not required
d) GBM allows for optimization on different loss functions 
...
partial_plot_3 <- partial(gbm_model_2, pred.var = "tourist_ind", 
                          n.trees = gbm_model_2$n.trees, 
                          plot = TRUE, plot.engine = "ggplot2") + 
  theme(
    text = element_text(size=16),
...
# Foreign nationals
foreign_nat <- df_model_rio %>% 
  group_by(tourist_ind) %>% 
  summarise(successful_bookings =mean(successful_booking, na.rm = TRUE))
  foreign_nat
```
